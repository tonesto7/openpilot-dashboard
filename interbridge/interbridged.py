#!/usr/bin/env python3
from selfdrive.interbridge.unisocket import MixedSocketServer
...
class InterBridge:
  ...
  def __init__(self, sm=None, pm=None, can_sock=None):
    ...
    # Setup sockets
    self.pm = pm if pm is not None else messaging.PubMaster(['testJoystick'])
    
    # Initialize an empty list for validated publishers
    validated_publishers = []

    # Output list of available services to console one service per line
    # print(f"Available services:")
    # for service in sorted(messaging.SERVICE_LIST):
    #   print(f"  {service}")

    # Check each service in the original publishers list
    for service in self.publishers:
      if service in messaging.SERVICE_LIST:
        validated_publishers.append(service)
      else:
        # Output a warning message if the service is not available to console
        print(f"Service '{service}' is not available in the current OpenPilot setup.")

    # Use the validated publishers list for SubMaster
    self.publishers = validated_publishers
    self.sm = sm if sm is not None else messaging.SubMaster(self.publishers)
...
def main(sm=None, pm=None, logcan=None):
  print("Interbridged process started")
